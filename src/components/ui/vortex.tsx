...
export const Vortex: React.FC<VortexProps> = ({
  ...
  const initParticle = (i: number) => {
    ...
    x = rand(canvas.width);
    y = center[1] + randRange(rangeY_);
    vx = 0;
    vy = 0;
    life = 0;
    ttl = baseTTL + rand(rangeTTL);
    speed = baseSpeed_ + rand(rangeSpeed_);
    radius = baseRadius_ + rand(rangeRadius_);
    hue = baseHue_ + rand(rangeHue);
  ...
  const drawParticle = (
    ...
    ctx.strokeStyle = `hsla(${hue},50%,60%,${fadeInOut(life, ttl)})`;
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(x2, y2);
    ctx.stroke();
